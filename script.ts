...
const gridContainer = document.getElementById('mainCont') as HTMLSpanElement;
const grid = document.getElementById("grid") as HTMLDivElement

function checkForErrors(): [boolean, string[]] {

    const grid: string[][] = gridTo2DArray()
...
function setSize(element: HTMLInputElement): void{
...
function createGrid(size: number) {
    ...
    mainContainter?.appendChild(gridContainer)
}
...
async function getGridFromBackend() {
    try {

        const userID: string = "thinking"
        const url: string = `http://127.0.0.1:8000/api/getGrid/${userID}`
        ...
        const grid: string[][] = data.grid; // Grab the "grid" field
...
window.onload = () => {
    ...
    const userID: string = "thinking"
    const url: string = `ws://127.0.0.1:8000/ws/${userID}`
    console.log(url)
    ...
    socket.onmessage = (event) => {
        ...
        try {
            const data: string[][] = JSON.parse(event.data); // Parse the 2D array
            renderBoard(data)
...
async function sendGridToBackend() {
    
    const message = gridTo2DArray(); // Get the 2D array from the grid

    let errorResults: [boolean, string[]] = checkForErrors()
